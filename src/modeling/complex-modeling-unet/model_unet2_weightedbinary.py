...
def weighted_bce(y_true, y_pred):
  weights = (y_true * 65.) + 1. #original problem had 59 times of one class appear more than the other -- I changed it to 9 times, since about 90% of the training is empty
...
def unet(input_size = (256,256,1)):
    with strategy.scope():
        ...
        model.compile(optimizer = Adam(learning_rate= 0.001), loss = weighted_bce, metrics = ['accuracy', # OG learning rate from code source is 1e-4
...
root_logdir = os.path.join(os.curdir,"modeling","unet_weightedbinary", "tflogs")
...
root_checkpointdir = os.path.join(os.curdir,"modeling","unet_weightedbinary", "checkpoints")
...
callbacks = [
keras.callbacks.ModelCheckpoint(run_checkpointdir(root_checkpointdir), mode='max', monitor='mean_io_u', save_freq='epoch', save_best_only=False),
...
model.fit(
    train_gen, 
    epochs=10, # 25 was originally selected - would take 48 hours
    callbacks=callbacks, 
    verbose=1,
