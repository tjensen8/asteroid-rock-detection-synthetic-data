"""
Baseline unet model with weighted class inputs.
"""
...
def unet(input_size = (256,256,1)):
    with strategy.scope():
        ...
        conv10 = Conv2D(1, 1, activation = 'sigmoid')(conv9) #changed to softmax activation from sigmoid

        model = Model(inputs=inputs,outputs=conv10)
        print(model.summary())
        print("Compile Model")
        # https://keras.io/api/optimizers/adam/
        model.compile(optimizer = Adam(learning_rate= 0.0001), loss = TverskyLoss, metrics = ['accuracy', 
...
root_logdir = os.path.join(os.curdir,"modeling","unet_TverskyLoss", "tflogs")
...
root_checkpointdir = os.path.join(os.curdir,"modeling","unet_TverskyLoss", "checkpoints")
...
keras.callbacks.TensorBoard(run_logdir(root_logdir)),
