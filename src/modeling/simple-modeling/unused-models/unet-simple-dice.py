...
def unet(input_size = (256,256,1)):
    with strategy.scope():
        ...
        model.compile(optimizer = Adam(learning_rate= 0.001), loss = dice_coef_loss, metrics = ['accuracy', # OG learning rate from code source is 1e-4
            keras.metrics.MeanIoU(num_classes=2), keras.metrics.Recall(),keras.metrics.Precision(), dice_coef])
...
root_logdir = os.path.join(os.curdir,"simple-modeling","unet-dice", "tflogs")
...
root_checkpointdir = os.path.join(os.curdir,"simple-modeling","unet-dice", "checkpoints")
