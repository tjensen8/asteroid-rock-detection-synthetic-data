...
root_logdir = os.path.join(os.curdir,"modeling","simple-modeling","unet-weighted-bce-trainval", "tflogs")
...
root_checkpointdir = os.path.join(os.curdir,"modeling","simple-modeling","unet-weighted-bce-trainval", "checkpoints")
...
with strategy.scope():
    ...
    model.compile(optimizer = Adam(learning_rate= 0.0001), loss = wbce, metrics = ['accuracy', # OG learning rate from code source is 1e-4
    ...
    for epoch in range(epochs):
        ...
        if epoch==0:
            ...
            model = load_model(f'{root_checkpointdir}/model-training-{epoch-1}.h5', custom_objects={'wbce': wbce})
            history = model.fit(
                train_generator,
                callbacks=callbacks,
                verbose=1,
