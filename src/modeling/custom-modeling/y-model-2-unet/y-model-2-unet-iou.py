"""
Simplified Unet approach without edge image segmentation.

Gutted the middle of the unet and collected together.
...
def unet(input_size = (256,256,1)):
    with strategy.scope():
        ...
        conv91 = Conv2D(2, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv9)
        ...
        model.compile(optimizer = Adam(learning_rate= 0.0001), loss = dyn_weighted_bincrossentropy, metrics = ['accuracy', # OG learning rate from code source is 1e-4
...
root_logdir = os.path.join(os.curdir,"modeling","results","custom-modeling","y-model-2-unet", "tflogs-iou")
...
root_checkpointdir = os.path.join(os.curdir,"modeling","results","custom-modeling","y-model-2-unet", "checkpoints","traintestrun-iou")
