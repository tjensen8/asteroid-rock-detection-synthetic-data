...
NEED TO ADD UPSAMPLE LAYER FOR BOTH. 
...
with strategy.scope():
    ...
    def dl_model(input_size = (256,256,1), input_processed_size=(256,256,1)):
        ...
        med_info = UpSampling2D(size=(2,2))(med_info)
        low_info = concatenate([conv3a, conv3b], axis=-1)
        low_info = UpSampling2D(size=(4,4))(low_info)

        final_info = concatenate([high_info, med_info, low_info],axis=-1)
    ...
    root_logdir = os.path.join(os.curdir,"modeling","results","custom-modeling","y-model-branches", "tflogs-iou")
    ...
    root_checkpointdir = os.path.join(os.curdir,"modeling","results","custom-modeling","y-model-branches", "checkpoints","traintestrun-iou")
    ...
    model.compile(optimizer = Adam(learning_rate= 0.0001), loss = iou_coef, metrics = ['accuracy', # OG learning rate from code source is 1e-4
