...
class artificial_lunar_landscapes(keras.utils.Sequence):
    ...
    def __init__(self, batch_size, image_size, image_processed_size, input_img_paths, input_img_processed_paths):
    ...
    def __getitem__(self, idx):
        ...
        for j, path in enumerate(batch_input_img_processed_paths):
            img_processed = keras.preprocessing.image.load_img(path, color_mode='grayscale')
            img_processed = np.array(img_processed)
            img_processed = filters.sobel(img_processed)
            img_processed = np.expand_dims(img_processed, axis=-1)
            if img_processed.shape != (480,720,1):
                print("IMAGE TO BE CHANGED: ", img_processed.shape)
                img_processed = tf.image.resize(img_processed, (480,720))
            img_std = (img_processed - np.min(img_processed)) / (np.max(img_processed) - np.min(img_processed))
            img_processed = img_std * (1 - 0) + 0    
            #img_processed = img_processed/255.
            #print("PREPROCESSED IMAGE SHAPE: ", img_processed.shape)
            x_processed[j] = img_processed
        #x_processed = per_image_standardization(x_processed)
        ###### Y
...
def generate_image_paths(
    ...
    process_img_paths = os.listdir(processed_dir)
    process_img_paths.sort(key=lambda f: int(re.sub('\D', '', f)))
    process_img_paths = [os.path.join(processed_dir, fname) for fname in process_img_paths]
    ...
    return input_dir, processed_dir, input_img_paths, process_img_paths
# %%
